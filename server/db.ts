...
export const client = DynamoDBDocument.from(
  new DynamoDBClient({
    ...
    credentials: {
      accessKeyId:
        process.env.NODE_ENV !== "production" ? "accessKey" : process.env.AWS_ACCESS_KEY_ID!,
      secretAccessKey:
        process.env.NODE_ENV !== "production" ? "secretKey" : process.env.AWS_SECRET_ACCESS_KEY!,
    },
    endpoint: process.env.NODE_ENV !== "production" ? "http://localhost:8000" : undefined,
  })
