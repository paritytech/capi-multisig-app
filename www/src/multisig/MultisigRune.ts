...
import { multisigAccountId } from "./multisigAccountId.js"
...
export class MultisigRune<out C extends Chain, out U>
  extends PatternRune<Multisig, C, U>
{
  ...
  threshold = this.value.map(({ threshold, signatories }) =>
    threshold ?? signatories.length - 1
  )
  accountId = Rune.fn(multisigAccountId).call(
    this.value.access("signatories"),
    this.threshold,
  )
  ...
  ratify<X>({ sender, call: call_ }: RunicArgs<X, MultisigRatifyProps<C>>) {
    const call = this.chain.extrinsic(
      Rune.resolve(call_).unsafeAs<Chain.Call<C>>(),
    )
      ...
      Rune
        .rec({
          ...
          value: Rune.rec({
            ...
            callHash,
            otherSignatories: this.otherSignatories(sender),
            timepoint: this.maybeTimepoint(callHash).map((x) =>
              x ?? new NoProposalError()
            ),
          }),
        })
        .unsafeAs<Chain.Call<C>>(),
  ...
  maybeTimepoint<X>(
    ...[callHash, blockHash]: RunicArgs<
      X,
      [callHash: Uint8Array, blockHash?: string]
    >
  ) {
    return this
  ...
  proposals<X>(
    ...
    return this.storage.keyPage(
      ...
      Rune
        ...
        .unsafeAs<
          $.Native<Chain.Storage<C, "Multisig", "Multisigs">["partialKey"]>
        >(),
      undefined,
      blockHash,
    )
    ...
    return this.storage
      .value(
        Rune
          ...
          .unsafeAs<
            $.Native<Chain.Storage<C, "Multisig", "Multisigs">["key"]>
          >(),
        blockHash,
      )
      .unsafeAs<
