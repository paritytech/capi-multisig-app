...
export function TransactionNew() {
  ...
  const onSubmit: SubmitHandler<FormData> = async (formDataNew) => {
    ...
    const selectedSetup = setups.peek().find((s) => s.multisig === formDataNew.from?.address);
    updateTransactionData({
      ...formDataNew,
      callHash,
      callData,
  ...
  return (
    <div className="flex flex-col gap-6 divide-y divide-divider">
      ...
      >
        <div class="flex flex-col gap-4">
          ...
          <div className="space-y-2">
            ...
            <Controller
              ...
              render={({ field }) => <AccountSelect {...field} accounts={fromAccounts} />}
            />
          </div>
          <div className="space-y-2">
            <p>To:</p>
        ...
        <div class="pt-4">
          <SumTable unit="WND">
            <SumTable.Row name="Send" value={transactionData.value.amount.toFixed(4)} />
            <SumTable.Row name="Transaction Fee" value={formatBalance(fee.value)} />
          </SumTable>
        </div>
